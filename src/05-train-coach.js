 ...
 *      - Example: isAnyWaitlisted([{status:"confirmed"}, {status:"passenger"}]) => true
 *
...
export function isAnyWaitlisted(passengers) {
  ...
  const checkWaitlist = (passenger) => passenger.status === "waitlisted"
  return passengers.some(checkWaitlist)
